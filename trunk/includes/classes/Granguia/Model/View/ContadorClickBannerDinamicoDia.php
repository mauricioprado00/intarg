<?
class Granguia_Model_View_ContadorClickBannerDinamicoDia extends Granguia_Db_Model_View_Generic{
	public function _construct(){
		parent::_construct();
		$tipo_contador = Granguia_Model_TipoContador::getTipoContadorByName('CLICK_BANNER_DINAMICO');
//		var_dump($this->getDb()->getLastErrors());
		$view = new Granguia_Db_Model_View_Generic();
		
		$view
			->addTable('gg_contador',null,'c',array('id_tipo', 'id_banner_dinamico'=>'info1', 'url'=>'info2'))
			...
			->setGroupBy('id_banner_dinamico,url,dia')
...
//			->addColumn('CONCAT('.($this->getDb()->nameToString('c.apellido')).', \', \', '.($this->getDb()->nameToString('c.nombre')).')', 'nombre_completo')
		...
		//echo "\n".$view->searchGetSql();
//		die();
		$this->addView($view, 'c', array('id_tipo','id_banner_dinamico','url', 'max_id', 'cantidad', 'dia'));
	}
}
?>