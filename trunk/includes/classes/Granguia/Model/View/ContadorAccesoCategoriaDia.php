<?
class Granguia_Model_View_ContadorAccesoCategoriaDia extends Granguia_Db_Model_View_Generic{
	public function _construct(){
		parent::_construct();
		$tipo_contador = Granguia_Model_TipoContador::getTipoContadorByName('ACCESO_CATEGORIA');
//		var_dump($this->getDb()->getLastErrors());
		$view = new Granguia_Db_Model_View_Generic();
		
		$view
			->addTable('gg_contador',null,'c',array('id_tipo', 'id_categoria'=>'info1', 'id_barrio'=>'info2'))
...
//			->addColumn('CONCAT('.($this->getDb()->nameToString('c.apellido')).', \', \', '.($this->getDb()->nameToString('c.nombre')).')', 'nombre_completo')
		...
		//echo "\n".$view->searchGetSql();
//		die();
		$this->addView($view, 'c', array('id_tipo','id_categoria', 'max_id', 'cantidad', 'dia'));
	}
}
?>