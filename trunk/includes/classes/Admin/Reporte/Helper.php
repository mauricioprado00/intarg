...
class Admin_Reporte_Helper extends Core_Singleton{
	...
	public static function actionAgregarEditarReporte($reporte){
		...
		if(!$reporte->hasId()){/** aca hay que agregar a la base de datos*/
			...
			if($resultado){
				Admin_App::getInstance()->addSuccessMessage(self::getInstance()->__t('Reporte añadida correctamente'));
			}
			else{
				Admin_App::getInstance()->addErrorMessage("No se pudo agregar la Reporte, error en la operación");
			}
		}
			...
			if($resultado){
				...
				Admin_App::getInstance()->addErrorMessage("No se pudo actualizar la Reporte, error en la operación");
			}
		}
		return($resultado);
	}
