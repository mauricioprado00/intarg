<?
class Admin_TipoPunto_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$tipo_punto->setWhere($comparator);
		}
		$datos = array();
		$total_items = $tipo_punto->searchCount();
...
//		}
		...
		//$datos = $tipo_punto->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $tipo_punto->search($sidx,$sord,$rows,$rows*($page-1));
