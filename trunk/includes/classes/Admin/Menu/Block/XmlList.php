<?
class Admin_Menu_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$menu = new Granguia_Model_Menu();
		if($comparator!=null){
			$menu->setWhere($comparator);
		}
		$datos = array();
		$total_items = $menu->searchCount();
...
//		}
		...
		//$datos = $menu->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $menu->search($sidx,$sord,$rows,$rows*($page-1),'Admin_Menu_Model_Menu');
