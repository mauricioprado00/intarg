<?
class Admin_Pagina_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$pagina->setWhere($comparator);
		}
		$datos = array();
		$total_items = $pagina->searchCount();
...
//		}
		...
		//$datos = $pagina->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $pagina->search($sidx,$sord,$rows,$rows*($page-1));
