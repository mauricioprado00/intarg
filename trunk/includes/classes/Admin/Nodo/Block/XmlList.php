<?
class Admin_Nodo_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$nodo->setWhere($comparator);
		}
		$datos = array();
		$total_items = $nodo->searchCount();
...
//		}
		...
		//$datos = $nodo->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $nodo->search($sidx,$sord,$rows,$rows*($page-1));
