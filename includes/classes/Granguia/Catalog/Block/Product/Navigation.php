<?
class Granguia_Catalog_Block_Product_Navigation extends Granguia_Catalog_Block_Product_List{
...
//	} 
	...
	private function initializeSearchObject(){
		if($this->search_object===null){
			...
			$wheres = array();
			$filtros = Granguia_Catalog_Block_Navigation::getFiltros();
...
//					$campo_busqueda = $post->getCampoBusqueda();
					if($texto_busqueda!==''){
						if(count($wheres))
							$wheres[] = ' AND ';
						if(strpos($campo_busqueda, '|')!==false){
							$campos_busqueda = explode('|',$campo_busqueda);
							$idx = 0;
							$wheres[] = '(';
							foreach($campos_busqueda as $campo_busqueda){
								if($idx++)
									$wheres[] = 'OR';
								$campo_busqueda = trim($campo_busqueda);
								$wheres[] = Db_Helper::like($campo_busqueda, '%', $texto_busqueda, '%');
							}
							$wheres[] = ')';
						}
						elseif($campo_busqueda==='' || !Granguia_Catalog_Model_Producto::isCampoBuscable($campo_busqueda)){
