<?
class Admin_Archivo_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$archivo->setWhere($comparator);
		}
		$datos = array();
		$total_items = $archivo->searchCount();
...
//		}
		...
		//$datos = $archivo->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $archivo->search($sidx,$sord,$rows,$rows*($page-1));
