<?
class Admin_Config_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		$datos = array();
		$total_items = $config->searchCount();
...
//		}
		...
		//$datos = $config->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $config->search($sidx,$sord,$rows,$rows*($page-1));
		//var_dump($config->search(null, 'ASC', null, 0, true, null, true));
