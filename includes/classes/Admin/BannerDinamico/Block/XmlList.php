<?
class Admin_BannerDinamico_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$banner_dinamico->setWhere($comparator);
		}
		$datos = array();
		$total_items = $banner_dinamico->searchCount();
...
//		}
		...
		//$datos = $banner_dinamico->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $banner_dinamico->search($sidx,$sord,$rows,$rows*($page-1));
