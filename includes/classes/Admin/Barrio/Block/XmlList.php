<?
class Admin_Barrio_Block_XmlList extends Jqgrid_Block_XmlList{
	protected function loadData($page, $rows, $sidx, $sord, $comparator){
		...
		*/
		...
		if($comparator!=null){
			$barrio->setWhere($comparator);
		}
		$datos = array();
		$total_items = $barrio->searchCount();
...
//		}
		...
		//$datos = $barrio->search(null,'ASC',null,0,true,array('id', 'username', 'nombre', 'apellido', 'activo', 'privilegios', 'ultimo_acceso')); 
		$datos = $barrio->search($sidx,$sord,$rows,$rows*($page-1));
